...
void TranspositionTable::store(uint64_t hash, int depth, int value, piecePosition bestMove, bool hasBestMove) {
...
bool TranspositionTable::probe(uint64_t hash, int depth, int alpha, int beta, TTEntry& entry) {
        ...
        if (entry.hash != hash || entry.depth < depth || entry.hasBestMove==false) {
            return false;
        }
